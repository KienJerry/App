...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callBack) => {
    ...
    let imgname =
      new Date().getTime().toString() + "." + myArray[myArray.length - 1]; // Đặt lại tên image thành date + time + .(đuôi ảnh)
...
app.get("/danhmucsanpham", function (req, res) {
  con.query(
    "SELECT * FROM `danhmucsanpham` order by madanhmucsanpham desc",
...
app.post("/deletedanhmucsanpham", function (req, res) {
  // console.log("abc")
  var sql =
    "delete from danhmucsanpham where madanhmucsanpham = (" +
    req.body.myid +
    ")";
  console.log(sql);
  con.query(sql, function (err, result, fields) {
...
app.post("/editdanhmucsanpham/editid", function (req, res) {
  ...
  var sql =
    "UPDATE danhmucsanpham SET tendanhmucsanpham = ('" +
    req.body.name_category +
    "'), idcha =('" +
    req.body.name_category_parent +
...
app.get("/thuonghieu", function (req, res) {
  con.query(
    "SELECT * FROM `thuonghieu` order by mathuonghieu desc",
    function (err, result, fields) {
      // console.log(result);
      if (err) throw err;
      res.send(result);
...
app.post("/deletethuonghieu", function (req, res) {
  // console.log("abc")
  var sql =
    "delete from thuonghieu where mathuonghieu = (" + req.body.myid + ")";
  console.log(sql);
  con.query(sql, function (err, result, fields) {
    if (err) throw err;
    if (result == "okdelete") {
...
app.post("/editthuonghieu/editid", function (req, res) {
  console.log("Vào server thành công");
  var sql =
    "UPDATE thuonghieu SET tenthuonghieu = ('" +
    req.body.name_trademark +
    "'), email =('" +
    req.body.name_trademark_email +
    "'), diachi =('" +
    req.body.name_trademark_address +
    "')  where mathuonghieu = (" +
    req.body.myid +
    ")";
  console.log(sql);
...
app.get("/hinhanh", function (req, res) {
  con.query(
    "SELECT * FROM `hinhanh`order by mahinhanh desc",
    function (err, result, fields) {
      // console.log(result);
      if (err) throw err;
      res.send(result);
...
app.post("/addsanphamimg", upload.single("file"), (req, res, next) => {
  ...
  if (!file) {
    ...
    var sql =
      ...
      req.body.giasanpham +
      "', '" +
      req.body.chitietsanpham +
      "', '" +
      req.body.hangsanxuat +
      "','" +
      file.filename +
      "' ,'" +
      req.body.thuonghieu +
...
app.post("/edituploadFileSanPham", upload.single("file"), (req, res, next) => {
  //upload.single('file') , 'file' lấy từ bên react js qua
  const file = req.file;
  if (!file) {
...
app.post("/upload-multiple-images", upload.array("file", 5), (req, res) => {
  ...
  for (i = 0; i < file.length; i++) {
    ...
    var imgsrc = "http://localhost:3001/images/" + file.filename;
    var sql =
      "insert into hinhanh (url , idsanpham )  values ('" +
      file[i].filename +
      "' , '" +
      req.body.idsanpham +
      "')";
...
app.post("/uploadFileAPI", upload.single("file"), (req, res, next) => {
  ...
  if (!file) {
    ...
    var imgsrc = "http://localhost:3001/images/" + file.filename;
    var sql =
      "insert into hinhanh (url, tenhinhanh)  values ('" +
      file.filename +
      "', '" +
      req.body.name +
      "')";
...
app.get("/hienthihinhanh", function (req, res) {
  // cái đầu là html :v viết dài dòng hơn , cái dưới là chạy bằng ejs , viết 1 dòng nhưng lại thêm nhiều file và folder
